function messages(parameters, server, database) {
    ...
    server.post('/messages/search', (req, res) => {
        const searchContent = req.body.content;

        if (!searchContent) {
            server.handleError(req, res, {
                'error': "The search input can't be empty.",
                'redirect': '/'
            });
            return;
        }
        const Messages = database.models.Message;
        const User = database.models.User;

        Messages.findAll({ 
            'include': [{'model': User, 'where' : {login: searchContent}}]
        }).then(messages => {
